...
#Loglike_LA.c <- function(lam0.a, gam.a, lam0.b, gam.b, lam0.c, gam.c, dd, uu, omage, alpha1t, alpha2t, osigma, M, Z, S, X){

 # sum1 <- 0
  #for (j in 1:G) {
    #part11 <- (uu[,j]*X[,4]+rep(alpha.lam0.c, G)-1)*log(lam0.c[j]) - (rep(beta.lam0.c, G) * lam0.c)     
   # tempe <- 0
   #     if (j < G) {
   #       for (g in (j + 1):G) {
   #         tempe <- tempe + uu[, g]*(dd[j + 1] - dd[j])
   #       }
  #      }

  #  part21 <- -lam0.c[j]*(X[,3]+X[,4])*(uu[,j]*(X[,2] - dd[j]) + tempe)*exp(cbind(Z, S, M) %*% as.vector(gam.a) + alpha2t*omage)   
 #   sum1 <- sum1 + sum(part11) + colSums(part21)  
 # } 
...
################################################################## 
 ...
 ################################################################## 
 #ARMS算法
 #    qq1=function(lam0.a, gam.a, lam0.b, gam.b, lam0.c, gam.c, lam1, r, dd, uu, omage, alpha1t, alpha2t, osigma, M, Z, S, X) {
 #         a1=(lam1>0)*(lam1<20);
 #         return(a1);
 #    }

 #    for(ww in 1:G){
  #        f7 <- function(lam0.a, gam.a, lam0.b, gam.b, lam0.c, gam.c, lam1, r, dd, uu, omage, alpha1t, alpha2t, osigma, M, Z, S, X){
 #         lam0.c[r] = lam1
   
 #         Q = Loglike_LA.c(lam0.a, gam.a, lam0.b, gam.b, lam0.c, gam.c, dd, uu, omage, alpha1t, alpha2t, osigma, M, Z, S, X) 
 #         return(Q)
 #    }

 #    gen7<-arms(y.start = runif(1,0,1), f7, qq1, 1, r = ww, 
 #               lam0.a=lam0.a, gam.a=gam.a, lam0.b=lam0.b, gam.b=gam.b, 
 #               lam0.c=lam0.c, gam.c=gam.c, dd=dd, uu=uu, 
 #               omage=omage, alpha1t=alpha1t, alpha2t=alpha2t, osigma=osigma,
 #               M=M, Z=Z, S=S, X=X)  
 #    lam0.c[ww] = gen7
 #    }
